// thermo3_skel.c
// Jesus Narvaez and Spencer Moss
// 11/6/18
...
TWCR = 0x84; //start the TWI write process
...
while(1){          //main while loop
  ...
  twi_start_rd(0X91, lm73_wr_buf, 2); //read temperature data from LM73 (2 bytes) 
  _delay_ms(2);    //wait for it to finish
  lm73_temp = TWDR; //save high temperature byte into lm73_temp
  lm73_temp = lm73_temp << 8; //shift it into upper byte 
  lm73_temp |= TWDR;//"OR" in the low temp byte to lm73_temp 
